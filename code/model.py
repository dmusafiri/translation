...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1) #  [[0], ..., [seq_len - 1]]
        denominator = torch.arange(0, d_model, 2).float() # (d_model)
        denominator = torch.pow(10000.0, 2 * denominator / d_model)
        pe[:, 0::2] = torch.sin(position/denominator)
        pe[:, 1::2] = torch.cos(position/denominator)

        pe = pe.unsqueeze(0) # Third dimension for batch

        self.register_buffer('pe', pe) # This way the tensor will be saved with the state of the model
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model:int, h: int, dropout: float) -> None:
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model is not a multiple of h"
...
class Transformer(nn.Module):
    def __init__(self, encoder:Encoder, decoder:Decoder, src_embedding: InputEmbeddings, tgt_embedding: InputEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer):
        super().__init__()
...
def build_transformer(src_vocab_size:int, tgt_vocab_size:int, src_seq_len:int, tgt_seq_len:int, d_model:int = 512, N:int = 6, h:int = 8, dropout:float = 0.1, d_ff = 2048) -> Transformer:
    ...
    # Initializing the parameter with the xavier
